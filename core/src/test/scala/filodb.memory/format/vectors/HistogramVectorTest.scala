...
class HistogramVectorTest extends NativeVectorTest {
  ...
  // ── DeltaHistogramReader.sum() tests (delta histogram query path) ──
  // These test the optimized sum() used by: rate(delta_hist[5m]) → SumOverTimeChunkedFunctionH

  it("DeltaHistogramReader sum should match expected bucket-wise totals") {
    val appender = HistogramVector.appending(memFactory, 1024)
    rawLongBuckets.foreach { rawBuckets =>
      BinaryHistogram.writeDelta(bucketScheme, rawBuckets, buffer)
      appender.addData(buffer) shouldEqual Ack
  ...
  it("DeltaHistogramReader sum of single histogram should equal that histogram") {
    val appender = HistogramVector.appending(memFactory, 1024)
    rawLongBuckets.foreach { rawBuckets =>
      BinaryHistogram.writeDelta(bucketScheme, rawBuckets, buffer)
      appender.addData(buffer) shouldEqual Ack
  ...
  it("DeltaHistogramReader sum of sub-range should match expected") {
    val appender = HistogramVector.appending(memFactory, 1024)
    rawLongBuckets.foreach { rawBuckets =>
      BinaryHistogram.writeDelta(bucketScheme, rawBuckets, buffer)
      appender.addData(buffer) shouldEqual Ack
  ...
  it("DeltaHistogramReader sum should work across section boundaries (>64 histograms)") {
    val numElements = 100
  ...
  it("DeltaHistogramReader sum should work with custom bucket schemes") {
    val appender = HistogramVector.appending(memFactory, 1024)
    customHistograms.foreach { custHist =>
      custHist.serialize(Some(buffer))
      appender.addData(buffer) shouldEqual Ack
  ...
  it("DeltaHistogramReader sum should handle zero-valued histograms") {
